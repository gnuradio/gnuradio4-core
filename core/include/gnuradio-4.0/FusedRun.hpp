...
struct FusedRun : BlockModel {
    ...
    bool                _dissolved    = false; ///< written by the scheduler under its execution-order mutex
    ...
    [[nodiscard]] work::Result work(std::size_t requestedWork) override {
        ...
        // a worker copies its job list once, so a dissolved run outlives the shared list it was taken out of
        if (isDissolved()) {
    ...
    void               dissolve() noexcept { gr::atomic_ref(_dissolved).store_release(true); }
    [[nodiscard]] bool isDissolved() noexcept { return gr::atomic_ref(_dissolved).load_acquire(); }
