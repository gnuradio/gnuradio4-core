...
def registration_units(stem, pending, split):
    """Group the registrations into the units that carry them."""
    units = []
    local_index = 0
...
def main(argv=None):
    ...
    print(
        "parsing header: '{}' -> '{}'  split: {}".format(
            header, out_dir, "Yes" if options.split else "No"
        )
    )
    ...
    # the split output declares the initializer as bool, the per-macro-line output as std::size_t
    declared_type = "bool" if options.split else "std::size_t"

    # declaration and definition in one: the unit names the block type, so it materialises it
    for unit_name, members in registration_units(stem, pending, options.split):
        ...
        write_file(
            ...
            'extern "C" {{ {declared} {init}(gr::BlockRegistry&); }}\n'
            "#endif // HEADER_GUARD_{init}_HPP\n".format(
                init=init, declared=declared_type
            ),
        )
        write_file(
