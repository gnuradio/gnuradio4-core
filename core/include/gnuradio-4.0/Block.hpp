...
public:
    ...
    // the block's own output parameters, held until a work call has an output span to publish them into
...
public:
    ...
    /// a fresh apply supersedes a value still pending for the same key
    ...
    work::Result work(std::size_t requestedWork = std::numeric_limits<std::size_t>::max()) noexcept {
        if constexpr (Derived::blockCategory != block::Category::NormalBlock) {
            ...
            if (result.status == gr::work::Status::DONE) {
                ...
                // on that call is what lets a shutdown travel against the stream: an upstream block recognizes
...
FusedFront fusedFront(void* rawBlock, std::size_t requestedWork) {
    ...
    // work() reads the port caches without refreshing them and relies on the previous call to have left them
    // dirty; a run that falls back to work() between two chunks would otherwise read this call's counts back
    on_scope_exit invalidateOnExit = [&block] {
