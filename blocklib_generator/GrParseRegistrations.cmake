...
# * MAX_PER_TU <N>: balanced chunking -> all registrations of the header (across macro lines) are packed into .cpp files
#   of at most N instantiations each. This both splits large cartesian expansions and merges small macro lines, bounding
#   per-TU compile time and memory.
#
...
extern \"C\" { std::size_t ${_init}(gr::BlockRegistry&); }
...
# with MAX_PER_TU every registration of the header is collected first and the units are emitted after the scan
set(GR_PR_ALL_TYPES "")
set(GR_PR_ALL_NAMES "")
...
while(NOT GR_PR_AT_END)
  ...
  while(GR_PR_COMBINATION_INDEX LESS GR_PR_COMBINATION_COUNT)
    ...
    if(MAX_PER_TU GREATER 0)
      list(APPEND GR_PR_ALL_TYPES "${GR_PR_TYPE}")
      list(APPEND GR_PR_ALL_NAMES "|${GR_PR_FINAL_NAME}")
    elseif(SPLIT)
      gr_pr_write_unit("${GR_PR_STEM}_${GR_PR_MACRO_COUNT}_${GR_PR_COMBINATION_INDEX}" "${GR_PR_TYPE}"
                       "|${GR_PR_FINAL_NAME}")
  ...
  if(NOT SPLIT
     AND NOT
         ...
         GREATER
         0)
    gr_pr_write_unit("${GR_PR_STEM}_${GR_PR_MACRO_COUNT}" "${GR_PR_TYPES}" "${GR_PR_NAMES}")
    math(EXPR GR_PR_FILE_COUNT "${GR_PR_FILE_COUNT} + 1")
  endif()
...
# balanced chunks: sizes differ by at most one, none exceeding MAX_PER_TU
list(LENGTH GR_PR_ALL_TYPES GR_PR_TOTAL)
  ...
  while(GR_PR_CHUNK LESS GR_PR_CHUNK_COUNT)
    ...
    while(GR_PR_TAKEN LESS GR_PR_CHUNK_SIZE)
      ...
      list(
        ...
        GR_PR_NAME)
      list(APPEND GR_PR_TYPES "${GR_PR_TYPE}")
      list(APPEND GR_PR_NAMES "${GR_PR_NAME}")
      math(EXPR GR_PR_NEXT_INDEX "${GR_PR_NEXT_INDEX} + 1")
      math(EXPR GR_PR_TAKEN "${GR_PR_TAKEN} + 1")
    endwhile()
    gr_pr_write_unit("${GR_PR_STEM}_${GR_PR_CHUNK}" "${GR_PR_TYPES}" "${GR_PR_NAMES}")
    math(EXPR GR_PR_FILE_COUNT "${GR_PR_FILE_COUNT} + 1")
