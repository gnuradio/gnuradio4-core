...
struct Sink : Block<Sink> {
    ...
    std::size_t            nStopCalls = 0UZ;
    std::size_t            nReceived  = 0UZ; ///< samples.size(), published for a reader in another thread
    ...
    work::Status processBulk(InputSpanLike auto& inSpan) {
        ...
        gr::atomic_ref(nReceived).store_release(samples.size());
        return work::Status::OK;
    }
};
...
// the gate is raised from the test thread, so a live edit lands at a sample count the test knows
struct GatedSource : Block<GatedSource> {
    PortOut<float> out;
    ...
    work::Status processBulk(OutputSpanLike auto& outSpan) {
        const std::size_t allowed = gr::atomic_ref(gate).load_acquire();
        const std::size_t n       = std::min(outSpan.size(), allowed > emitted ? allowed - emitted : 0UZ);
        if (n == 0UZ) {
            outSpan.publish(0UZ);
            return work::Status::INSUFFICIENT_OUTPUT_ITEMS;
        }
        for (std::size_t i = 0UZ; i < n; ++i) {
            outSpan[i] = static_cast<float>(emitted + i);
        }
        outSpan.publish(n);
        gr::atomic_ref(emitted).store_release(emitted + n);
        return work::Status::OK;
    }
};

/// the traversal witness: it never starves, so PassCounter is visited on every pass over the job list
struct FreeSource : Block<FreeSource> {
    PortOut<float> out;

    GR_MAKE_REFLECTABLE(FreeSource, out);

    [[nodiscard]] constexpr float processOne() const noexcept { return 1.0f; }
};

struct PassCounter : Block<PassCounter> {
    PortIn<float> in;

    GR_MAKE_REFLECTABLE(PassCounter, in);

    std::size_t nPasses = 0UZ;

    work::Status processBulk(InputSpanLike auto& inSpan) {
        std::ignore = inSpan.consume(inSpan.size());
        gr::atomic_ref(nPasses).store_release(nPasses + 1UZ);
        return work::Status::OK;
    }
};
...
enum class LiveEdit { removeInteriorEdge, addInteriorFanOut };

struct LiveEditArm {
    std::vector<float> samples;    ///< the chain's own sink
    std::vector<float> tapSamples; ///< the consumer of the interior port, connected by the add edit
    std::size_t        nAtEdit   = 0UZ;
    std::size_t        nRuns     = 0UZ;
    bool               dissolved = false;
};

inline constexpr std::size_t kEditAt        = 1024UZ; // what the gate admits before the edit
inline constexpr std::size_t kAfterEdit     = 3072UZ; // what it admits after it
inline constexpr std::size_t kWitnessPasses = 256UZ;

inline void sendSchedulerMessage(gr::MsgPortOut& port, std::string_view endpoint, gr::property_map data) { gr::sendMessage<gr::message::Command::Set>(port, "", endpoint, std::move(data)); }

[[nodiscard]] inline bool awaitReply(gr::MsgPortIn& port, std::string_view endpoint) {
...
[[nodiscard]] inline bool awaitCount(std::size_t& counter, std::size_t target) {
    for (std::size_t i = 0UZ; i < 2000UZ; ++i) {
        if (gr::atomic_ref(counter).load_acquire() >= target) {
            return true;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
...
/**
 * @brief One arm of a live edit at an interior port of a fused run, edited through the scheduler's message plane.
 *
 * The gate fixes the sample count the edit lands on: the chain is drained before the message is sent and the gate
 * is only raised afterwards. The witness chain shares the job list with the run without belonging to it, so its
 * pass count proves the scheduler kept traversing after the edit -- which is what makes the removal arm's silence
 * a result rather than a stalled test.
 */
[[nodiscard]] inline LiveEditArm runLiveEditChain(LiveEdit edit, bool fusion, std::size_t chunkSamples) {
    using namespace boost::ut;
    const gr::EdgeParameters edge{.minBufferSize = 65536UZ};
    const std::string        what = std::format("{} fusion={} chunk={}", edit == LiveEdit::removeInteriorEdge ? "remove" : "add", fusion, chunkSamples);

    gr::Graph flow;
    auto&     source  = flow.emplaceBlock<GatedSource>(gr::property_map{{"name", std::string("src")}});
    auto&     gain    = flow.emplaceBlock<Gain>(gr::property_map{{"name", std::string("gain")}});
    auto&     quant   = flow.emplaceBlock<Quantize>(gr::property_map{{"name", std::string("quant")}});
    auto&     sink    = flow.emplaceBlock<Sink>(gr::property_map{{"name", std::string("snk")}});
    auto&     tap     = flow.emplaceBlock<Sink>(gr::property_map{{"name", std::string("tap")}});
    auto&     witness = flow.emplaceBlock<FreeSource>(gr::property_map{{"name", std::string("witness")}});
    auto&     passes  = flow.emplaceBlock<PassCounter>(gr::property_map{{"name", std::string("passes")}});
    expect(flow.connect<"out", "in">(source, gain, edge).has_value());
    expect(flow.connect<"out", "in">(gain, quant, edge).has_value());
    expect(flow.connect<"out", "in">(quant, sink, edge).has_value());
    expect(flow.connect<"out", "in">(witness, passes, edge).has_value());

    const std::string gainName  = gain.unique_name;
    const std::string quantName = quant.unique_name;
    const std::string tapName   = tap.unique_name;
    gr::atomic_ref(source.gate).store_release(kEditAt);
    ...
    LiveEditArm                 arm;
    std::shared_ptr<BlockModel> runEntry; // held so the entry the worker copied outlives its removal from the shared list
    for (const auto& job : *scheduler.jobs()) {
        for (const auto& entry : job) {
            if (dynamic_cast<fusion::FusedRun*>(entry.get()) != nullptr) {
                runEntry = entry;
                arm.nRuns++;
            }
        }
    ...
    expect(awaitCount(sink.nReceived, kEditAt)) << what << ": the chain did not drain before the edit";

    if (edit == LiveEdit::removeInteriorEdge) {
        sendSchedulerMessage(toScheduler, gr::scheduler::property::kRemoveEdge,                     //
            {{std::pmr::string(gr::serialization_fields::EDGE_SOURCE_BLOCK), gainName},             //
                {std::pmr::string(gr::serialization_fields::EDGE_SOURCE_PORT), std::string("out")}, //
                {std::pmr::string(gr::serialization_fields::EDGE_DESTINATION_BLOCK), quantName},    //
                {std::pmr::string(gr::serialization_fields::EDGE_DESTINATION_PORT), std::string("in")}});
        expect(awaitReply(fromScheduler, gr::scheduler::property::kEdgeRemoved)) << what << ": the edge was never removed";
    } else {
        sendSchedulerMessage(toScheduler, gr::scheduler::property::kEmplaceEdge,                        //
            {{std::pmr::string(gr::serialization_fields::EDGE_SOURCE_BLOCK), gainName},                 //
                {std::pmr::string(gr::serialization_fields::EDGE_SOURCE_PORT), std::string("out")},     //
                {std::pmr::string(gr::serialization_fields::EDGE_DESTINATION_BLOCK), tapName},          //
                {std::pmr::string(gr::serialization_fields::EDGE_DESTINATION_PORT), std::string("in")}, //
                ...
                {std::pmr::string(gr::serialization_fields::EDGE_NAME), std::string("interior tap")}});
        expect(awaitReply(fromScheduler, gr::scheduler::property::kEdgeEmplaced)) << what << ": the edge was never emplaced";
    }

    arm.nAtEdit   = gr::atomic_ref(sink.nReceived).load_acquire();
    arm.dissolved = runEntry != nullptr && dynamic_cast<fusion::FusedRun*>(runEntry.get())->isDissolved();

    const std::size_t passesAtEdit = gr::atomic_ref(passes.nPasses).load_acquire();
    gr::atomic_ref(source.gate).store_release(kEditAt + kAfterEdit);
    expect(awaitCount(passes.nPasses, passesAtEdit + kWitnessPasses)) << what << ": the scheduler stopped traversing the job list";

    if (edit == LiveEdit::addInteriorFanOut) {
        expect(awaitCount(sink.nReceived, kEditAt + kAfterEdit)) << what << ": the chain stopped delivering across the edit";
        expect(awaitCount(tap.nReceived, kAfterEdit)) << what << ": the consumer added at the interior port was never served";
    }
...
const boost::ut::suite<"fusion"> _fusion = [] {
    ...
    "a removed interior edge stops carrying samples"_test = [] {
        const LiveEditArm unfused = runLiveEditChain(LiveEdit::removeInteriorEdge, false, 0UZ);
        expect(eq(unfused.nRuns, 0UZ));
        expect(eq(unfused.nAtEdit, kEditAt));
        expect(eq(unfused.samples.size(), kEditAt)) << "unfused: nothing crosses the removed edge";

        for (const std::size_t chunk : {256UZ, 4096UZ}) {
            const LiveEditArm fused = runLiveEditChain(LiveEdit::removeInteriorEdge, true, chunk);
            const std::string what  = std::format("chunk={}", chunk);
            expect(eq(fused.nRuns, 1UZ)) << what;
            expect(fused.dissolved) << what << "the edit must retire the run the worker still holds";
            expect(eq(fused.nAtEdit, kEditAt)) << what;
            expect(eq(fused.samples.size(), unfused.samples.size())) << what << "a stale run keeps carrying samples through scratch";
            expect(std::ranges::equal(fused.samples, unfused.samples)) << what << "the stream up to the edit differs";
        }
    };

    "a consumer added at an interior port is served"_test = [] {
        const LiveEditArm unfused = runLiveEditChain(LiveEdit::addInteriorFanOut, false, 0UZ);
        expect(eq(unfused.nRuns, 0UZ));
        expect(eq(unfused.samples.size(), kEditAt + kAfterEdit));
        expect(eq(unfused.tapSamples.size(), kAfterEdit)) << "unfused: the new consumer gets what the port produces after the edit";

        for (const std::size_t chunk : {256UZ, 4096UZ}) {
            const LiveEditArm fused = runLiveEditChain(LiveEdit::addInteriorFanOut, true, chunk);
            const std::string what  = std::format("chunk={}", chunk);
            expect(eq(fused.nRuns, 1UZ)) << what;
            expect(fused.dissolved) << what;
            expect(eq(fused.nAtEdit, kEditAt)) << what;
            expect(std::ranges::equal(fused.samples, unfused.samples)) << what << "the chain's own stream differs across the edit";
            expect(std::ranges::equal(fused.tapSamples, unfused.tapSamples)) << what << "a stale run bypasses the interior port and starves the new consumer";
        }
    };
