...
void sendReplacementRequest(gr::MsgPortOut& port, const RefusalPassThrough& block) {
    gr::sendMessage<gr::message::Command::Set>(port, "", gr::scheduler::property::kReplaceBlock,
        {{"uniqueName", std::string(block.unique_name)}, {"type", gr::meta::type_name<RefusedCandidate>()}, {"properties", gr::property_map{{"gain", 3.0f}}}});
}
...
const boost::ut::suite<"scheduler replacement release refusal"> schedulerReplacementRefusalTests = [] {
    ...
    "a running graph continues and stops normally after replacement is refused"_test = [] {
        ...
        expect(qa_sched::awaitCondition([callsAfterRefusal] { return qa_sched::gRefusalWorkCalls.load(std::memory_order_relaxed) > callsAfterRefusal; }))
            << "A must continue processing after the refusal";
